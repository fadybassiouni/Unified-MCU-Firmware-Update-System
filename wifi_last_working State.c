...
/*ESP Prototypes*/
void HESP_voidSendCommand(const char *ARG_pccharCommand);
HESP_status HESP_hespstatusGetResponse( u16 ARG_u16Timeout, u8 ARG_u8ResponseType);
HESP_status HESP_hespstatusSendAndExpectResponse(const char *ARG_pccharCommand, const char *ARG_pccharExpectedResponse, u16 ARG_u16Timeout, u8 ARG_u8ResponseType);
...
HESP_status HESP_hespstatusSendToServer(const char *ARG_pccharCommand);
void HESP_voidGetServerResponse(char *ARG_pccharResponse);
void HESP_voidTimeout(void);
void HESP_voidFlushBuffer(void);
void HESP_writeToBuffer(void);

/*ESP Buffer & Version Array*/
char ESP_charBuffer[200]={0};
char charVersion[8]={1};
u8 currentState=0;
u8 HESP_u8Timeout = 0;
u8 HESP_u8Iterator = 0;
...
int main(void)
{
	...
	/*UART1 and UART2 AF GPIO Mode Change*/
	...
	MUART_stderrInit(MUART_UART1, 0, MUART_DATABITS_8, MUART_PARITY_NONE, MUART_STOPBITS_1);
	...
	if(currentState == HESP_STATE_ACTIVE)
	{
	...
	else
	{
		...
		MUART_voidSendChar(MUART_UART2, currentState);
		MUART_voidSendChar(MUART_UART2, '\n');
		MUART_voidSendString(MUART_UART2, "Error Dump: ");
		MUART_voidSendString(MUART_UART2, ESP_charBuffer);
		MUART_voidSendChar(MUART_UART2, '\n');
	}

	/*Get AT Version*/
	currentState = HESP_hespstatusGetVersion(charVersion);
	if(currentState == HESP_VERSION_AVAILABLE)
	{
		MUART_voidSendString(MUART_UART2, "AT Ver: ");
		MUART_voidSendString(MUART_UART2, charVersion);
		MUART_voidSendChar(MUART_UART2, '\n');
	}
	else
	{
	...
	delay(100000);
	if(currentState == HESP_OP_MODE_SET)
	{
	...
	//delay(100000);
	currentState = HESP_hespstatusConnectToWIFI("Paradox", "tryagainlaterin2020");

	if(currentState == HESP_WIFI_CONNECTED)
	{
	...
	{
		/*Dump Error Code to LCD (For Debugging Purposes)*/
		MUART_voidSendString(MUART_UART2, "Error Code: ");
		MUART_voidSendChar(MUART_UART2, currentState);
		MUART_voidSendChar(MUART_UART2, '\n');
	}
	...
	/*Connect To Server*/
	currentState = HESP_hespstatusConnectToServer(HESP_CONNECTION_MODE_SINGLE, HESP_SERVER_PROTOCOL_TCP, "192.168.1.11", 55555);
	if(currentState == HESP_SERVER_CONNECTED)
	{
		MUART_voidSendString(MUART_UART2, "Server Connected\n");
	}
	else
	{
		/*Dump Error Code to UART (For Debugging Purposes)*/
	...
	char HESP_pcharResponse[201];
	char *HESP_pcharHEXStartIndex = NULL_POINTER;
	char *HESP_pcharEndIndex = NULL_POINTER;
	char *HESP_pcharDone = NULL_POINTER;
	currentState = HESP_hespstatusSendToServer("start");
	u8 i=1;
	while(i)
	{
		if(currentState == HESP_SERVER_SEND_SUCCESS)
		{
			HESP_voidGetServerResponse(HESP_pcharResponse);
			HESP_pcharHEXStartIndex = strstr(HESP_pcharResponse, "::");
			HESP_pcharDone = strstr(HESP_pcharResponse, "Done");
			HESP_pcharEndIndex = strstr(HESP_pcharResponse, "EOT");
			if((HESP_pcharHEXStartIndex != NULL_POINTER) && (HESP_pcharEndIndex != NULL_POINTER))
			{
				MUART_voidSendString(MUART_UART2, HESP_pcharHEXStartIndex);
				currentState = HESP_hespstatusSendToServer("next");
			}
			else if((HESP_pcharDone != NULL_POINTER) && (HESP_pcharEndIndex != NULL_POINTER))
			{
				i=0;
			}
			else
			{
				MUART_voidSendString(MUART_UART2, "IPD Not Found\n");
				currentState = HESP_hespstatusSendToServer("again");
			}
		}
		else
		{
//			MUART_voidSendString(MUART_UART2, "------------------------------------------------\n");
			MUART_voidSendString(MUART_UART2, "Server Send Fail\n");
//			MUART_voidSendString(MUART_UART2, ESP_charBuffer);
//			MUART_voidSendChar(MUART_UART2, '\n');
//			MUART_voidSendString(MUART_UART2, "------------------------------------------------\n");

			if(strstr(ESP_charBuffer, "ERROR") != NULL_POINTER)
			{
				currentState = HESP_hespstatusSendToServer("next");
			}
			else
...
void HESP_writeToBuffer(void)
{
	...
	ESP_charBuffer[HESP_u8Iterator] = MUART_voidRecieveCharBlocking(MUART_UART1);
	HESP_u8Iterator++;
}
...
{
	/*Flush Buffer*/
	for(HESP_u8Iterator = 0; HESP_u8Iterator<200; HESP_u8Iterator++)
	{
		ESP_charBuffer[HESP_u8Iterator]='\0';
	}
...
HESP_status HESP_hespstatusGetResponse(u16 ARG_u16Timeout, u8 ARG_u8ResponseType)
{
	...
	for(;;)
	{
		...
		{
			if((strstr(ESP_charBuffer, "OK") != NULL_POINTER) || (strstr(ESP_charBuffer, "ERROR") != NULL_POINTER))
			{
		...
		{
			if((strstr(ESP_charBuffer, "OK") != NULL_POINTER) || (strstr(ESP_charBuffer, "FAIL") != NULL_POINTER))
			{
				L_hespstatusState = HESP_RESPONSE_RECIEVED;
				break;
			}
		...
		{
			if(strstr(ESP_charBuffer, "EOT") != NULL_POINTER)
			{
				L_hespstatusState = HESP_RESPONSE_RECIEVED;
				break;
			}
...
HESP_status HESP_hespstatusSendAndExpectResponse(const char *ARG_pccharCommand, const char *ARG_pccharExpectedResponse, u16 ARG_u16Timeout, u8 ARG_u8ResponseType)
{
	...
	if(L_hespstatusState == HESP_RESPONSE_RECIEVED)
	{
		...
		if(strstr(ESP_charBuffer, ARG_pccharExpectedResponse) != NULL_POINTER)
		{
...
HESP_status HESP_hespstatusGetVersion(char *ARG_pcharVersion)
{
	...
	if(L_hespstatusState == HESP_RESPONSE_EXPECTED)
	{
		...
		L_pcharVersionStartIndex = strstr(ESP_charBuffer,"AT ver");
		u8 L_u8Iterator=0;
...
HESP_status HESP_hespstatusConnectToWIFI(const char *ARG_pccharssid, const char *ARG_pccharPassword)
{
	...
	/*Send Combined Command And Expect OK Response, Timout after 10 Seconds*/
	L_hespstatusState = HESP_hespstatusSendAndExpectResponse(L_combinedCommand, "\r\nOK", 16777, HESP_RESPONSE_TYPE_WIFI);
	...
	else if(L_hespstatusState == HESP_RESPONSE_UNEXPECTED)
	{
		...
		L_pcharStartIndex = strstr(ESP_charBuffer, "+CWJAP:");
...
HESP_status HESP_hespstatusSetConnectionMode(u8 ARG_u8ConnectionMode)
{
	...
	{
	case HESP_CONNECTION_MODE_SINGLE  : L_hespstatusState = HESP_hespstatusSendAndExpectResponse("AT+CIPMUX=0", "OK", 200, HESP_RESPONSE_TYPE_SYSTEM); break;
	case HESP_CONNECTION_MODE_MULTIPLE: L_hespstatusState = HESP_hespstatusSendAndExpectResponse("AT+CIPMUX=1", "OK", 200, HESP_RESPONSE_TYPE_SYSTEM); break;
...
HESP_status HESP_hespstatusSendToServer(const char *ARG_pccharCommand)
{
	...
	if(L_hespstatusState == HESP_RESPONSE_EXPECTED)
	{
		L_hespstatusState = HESP_hespstatusSendAndExpectResponse(ARG_pccharCommand, "EOT", 15000, HESP_RESPONSE_TYPE_SERVER);
		switch(L_hespstatusState)
		{
...
void HESP_voidGetServerResponse(char *ARG_pccharResponse)
{
	strcpy(ARG_pccharResponse, ESP_charBuffer);
}
