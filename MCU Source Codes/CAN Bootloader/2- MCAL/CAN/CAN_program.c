...
u8 CAN_voidGetRxMsg(u32 local_u8RxFifo, CAN_RxHeaderTypeDef *pRxHeader, u8 *Local_u8Data)
{
    ...
    /* Get the data */
		#if 0
    Local_u8Data[0] = (u8)((CAN->FIFOMailBox[local_u8RxFifo].RDLR 	  ) & 0XFF);
    Local_u8Data[1] = (u8)((CAN->FIFOMailBox[local_u8RxFifo].RDLR >> 8 ) & 0XFF);
    Local_u8Data[2] = (u8)((CAN->FIFOMailBox[local_u8RxFifo].RDLR >> 16) & 0XFF);
    Local_u8Data[3] = (u8)((CAN->FIFOMailBox[local_u8RxFifo].RDLR >> 24) & 0XFF);
...
void CAN_voidConfigFilter( CAN_FilterTypeDef *pFilterConfig)
{
	...
	/* Filter Deactivation */
	CLEAR_BIT(CAN->FA1R, pFilterConfig->FilterBank);

	/* Filter Scale */
	if (pFilterConfig->FilterScale == CAN_FILTERSCALE_16BIT)
	{
		/* 16-bit scale for the filter */
		CLEAR_BIT(CAN->FS1R, local_u32FilterBitPos);
	...
	if (pFilterConfig->FilterMode == CAN_FILTERMODE_IDMASK)
	{
		...
		CLEAR_BIT(CAN->FM1R, local_u32FilterBitPos);
	}
	else
	{
	...
	if (pFilterConfig->FilterFIFOAssignment == CAN_FILTER_FIFO0)
	{
		...
		CLEAR_BIT(CAN->FFA1R, local_u32FilterBitPos);
	}
	else
	{
...
{
	u32 CAN_ErrorCode  = CAN_ERROR_NONE;
	u32 interrupts = READ_REG(CAN->IER);
	u32 rf0rflags = READ_REG(CAN->RF0R);
...
void CAN_voidRX_FIFO1_Interrupt_Handler(void)
{
	u32 CAN_ErrorCode  = CAN_ERROR_NONE;
	u32 interrupts = READ_REG(CAN->IER);
	u32 rf1rflags = READ_REG(CAN->RF1R);
...
void CAN_voidSCE_Interrupt_Handler(void)
{
	CAN_ErrorCode  = CAN_ERROR_NONE;
	u32 interrupts = READ_REG(CAN->IER);
	u32 msrflags  = READ_REG(CAN->MSR);
	u32 esrflags = READ_REG(CAN->ESR);
...
__attribute__((weak)) void CAN_TxMailbox0CompleteCallback()
{
...
__attribute__((weak)) void CAN_ErrorCallback()
{
